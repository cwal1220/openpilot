...
from tools/orangepi5_python_requirements.txt.
EOF
}
...
if [ -f "$ROOT/selfdrive/modeld/runners/onnx_runner.py" ]; then
  chmod +x "$ROOT/selfdrive/modeld/runners/onnx_runner.py"
fi
...
echo "  python3 -c 'import capnp, zmq, usb1, serial, sentry_sdk, tqdm, crcmod, cffi, onnx'"
echo "  cd $ROOT && OPENPILOT_BUILD_PLATFORM=linux_generic USE_WEBCAM=1 NOSENSOR=1 scons -j$(nproc) selfdrive/camerad/camerad selfdrive/modeld/_modeld"
